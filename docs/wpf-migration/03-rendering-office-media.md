...
| `IMediaPlaybackBackend` | WPF `MediaElement`/DirectShow 등 실제 재생 엔진 adapter 경계, backend 오류의 상태 snapshot 전환 |
...
| 이미지 프리뷰 WPF 컨트롤 | 1차 구현 완료 | `IImageAssetService`, `ImageAssetService`, `PreviewCanvas`, `SlidePreviewControl` 추가. legacy `ImageCanvas.ResizeCanvas`와 같은 fit 중앙 정렬 계약, fill/stretch/center 배치, 이미지 메타데이터 로드, unsupported/locked/decode 오류 분류, bitmap pixel dimension 기반 DPI 안전 배치, `Source`/`FillMode`/선택 테두리/슬라이드 번호 WPF 렌더링 자동 검증 완료. `SlidePreviewControl`로 이미지 source가 있으면 `PreviewCanvas`를 표시하고, 없으면 title/kind fallback을 표시하며, MainWindow Preview 탭을 `LiveQueueItem` preview 속성에 연결했다. WinForms/WPF 이미지 diff는 후속 필요 |
| 전환 효과 WPF화 | 부분 구현 | `ITransitionEffectService`, `TransitionEffectService` 추가. legacy `ImageTransitionControl`의 58개 전환 이름/순서, `TransitionAction`, background layer 정책, progress clamp, fade/slide/reveal/stretch/zoom/spin/flip frame 계약 자동 검증 완료. output renderer scene frame 연결은 완료했으며, OutputWindow ContentArea에 Scene 변경 시 `DoubleAnimation` 기반 fade-in을 적용해 진입 애니메이션 1차 연결. 실제 cross-fade/slide/zoom 등 풀 transition visual 적용은 후속 필요 |
| 미디어 playback WPF화 | 부분 구현 | `IMediaPlaybackService`, `MediaPlaybackService`, `MediaPlaybackViewModel`, `IMediaPlaybackBackend`, `NoOpMediaPlaybackBackend`, `WpfMediaElementPlaybackBackend`로 재생 상태 계약, command, 시간 표시, seek/audio setting clamp, settings 기반 volume/balance/mute default 및 변경 이벤트 반영, `MediaDir` 기반 상대 파일 source 해석, `LiveCamNumber` 기반 `<<Capture>>n` live camera source 및 변경 이벤트 재로드, backend 명령 위임, 모든 주요 backend 명령 실패 시 `Failed` snapshot 전환, WPF `MediaElement` 파일 adapter 경계 자동 검증 완료. DirectShow adapter, 출력 화면 visual host 연결, 실제 fixture 파일/코덱 오류 UI 통합 검증은 후속 필요 |
| 출력 renderer 동등성 | 부분 구현 | `IOutputRenderer`, `OutputRenderer`, `OutputSceneSnapshot` 추가. `LiveSessionSnapshot`/`OutputWindowState`에서 Live/Hidden/Blackout/Ready/Standby 장면, 출력 모니터명, 표시 라벨, blackout flag, viewport, content placement, transition frame, PowerPoint/media panel overlay visibility를 생성하고 `OutputWindowViewModel.Scene`으로 바인딩 경계를 고정했다. `LiveSessionSnapshot.CurrentItemPreviewSource`/`PreviewFillMode`/`PreviewPixelWidth`/`PreviewPixelHeight`를 OutputWindow Canvas Image로 직접 송출하고, Blackout/Hidden 시 검정 강제 오버레이로 사용자 배경/이전 콘텐츠 누출 방지. 실제 WinForms/WPF 이미지 diff와 full output visual renderer는 후속 필요 |
...
- 현재 1차 자동 검증은 `IMediaPlaybackBackend` fake/NoOp 기반 상태 계약과 WPF `MediaElement` adapter 경계로 수행한다. 실제 playback visual host 연결 후 fixture 파일 기반 통합 검증을 추가한다.
...
- `AppServiceRegistrationTests`: WPF 운영 DI 등록이 `PowerPointRenderService`와 `WindowPlacementService`를 settings-backed 생성자로 resolve하는지 검증
...
- `SlidePreviewControlTests`: preview source를 `PreviewCanvas`에 전달, source 없음 title/kind fallback, MainWindow Preview 탭의 `SelectedItem.PreviewSource`/`PreviewFillMode`/`SlideNumber` 바인딩 검증
...
- `dotnet test Easislides.Wpf.Tests\Easislides.Wpf.Tests.csproj -c Debug --filter SlidePreviewControlTests`: 3개 통과
- `dotnet test Easislides.Wpf.Tests\Easislides.Wpf.Tests.csproj -c Debug --filter TransitionEffectServiceTests`: 7개 통과
- `dotnet test Easislides.Wpf.Tests\Easislides.Wpf.Tests.csproj -c Debug --filter "OutputRendererTests|OutputWindowViewModelTests"`: 21개 통과
