...
| `PraiseBookItems` | `PraiseBookItems` (`Tag=PraiseBookItems`) | partial | 3 | flat headerless ListView opens legacy `.esp` items with `SongId` from `ItemID`; selecting a row now resolves the song and refreshes the right Preview item without adding it to Worship List, while double-click/Enter add-to-Worship plus drag insert into Worship List use the same DB-backed `SongId` resolution; exact legacy list density/sort behavior still incomplete |
