...
| `IndPanel`, `Ind_*` | `IndPanel` (`Tag=IndPanel`), `Ind_checkBox`, `IndgroupBox1..4`, `Ind_BackColour`, `Ind_BackImageSelect`, `Ind_NoImage`, `IndcbPreviewNotes`, `IndradioButtonText/Format/Info` | partial | 5 | Preview Set mode now exposes first-screen individual-format toggle plus item text color/effect/alignment/size/font/background/copy/clear commands; item background image path/select/clear is wired through `SetSelectedItemBackgroundImageCommand`/FormatData 61; `IndcbPreviewNotes` reuses the same current-session notes editor as Tools and `WL_Notes`; default/template/image-mode details still incomplete |
