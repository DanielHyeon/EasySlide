...
| `flowLayoutPreviewLyrics` | `flowLayoutPreviewLyrics` (`Tag=flowLayoutPreviewLyrics`) plus lyrics text | partial | 4/6 | Text mode now exposes the legacy lyrics surface role; focused Preview surface accepts keyboard focus and routes page/verse keys; exact click/highlight/page behavior still incomplete |
