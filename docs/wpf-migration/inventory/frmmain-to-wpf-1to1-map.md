...
| `cbOutputBlack` | `cbOutputBlack` + bottom/menu/F9 Output Black toggle + `ToggleOutputBlackCommand` / `IsOutputBlackActive` | partial | 4/5/6 | top, bottom, menu, F9, and palette paths now behave as checked toggles that call Black and restore on second click; exact legacy remote/icon flash sync still incomplete |
...
| Global live shortcuts | `KeyboardActionHandler`, hook handlers | partial | 6 | F12/F9/F3 command registry paths now use the same checked toggle commands as the visible FrmMain-style controls; F7/F8/F10/global hook option parity and remaining focus edge cases still incomplete |
