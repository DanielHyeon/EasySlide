...
| `IndPanel`, `Ind_*` | `IndPanel` (`Tag=IndPanel`), `Ind_checkBox`, `IndgroupBox1..4`, `IndcbPreviewNotes`, `IndradioButtonText/Format/Info` | partial | 5 | Preview Set mode now exposes first-screen individual-format toggle plus item text color/effect/alignment/size/font/background/copy/clear commands; `IndcbPreviewNotes` reuses the same current-session notes editor as Tools and `WL_Notes`; default/template/background-image details still incomplete |
