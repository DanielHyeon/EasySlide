...
| `PreviewHolder`, `PreviewBack` | `ClassicPreviewHolder` (`Tag=PreviewHolder`) / `ClassicPreviewBack` (`Tag=PreviewBack`) with `PreviewLyricsText` and visual-source overlay | partial | 4 | holder/back roles are explicit and lower Preview now scales a fixed 960x540 frame; song/Bible/notice Preview pages render from `PreviewLyricsText`, while PPT/image PreviewSource overlays only when a visual source exists; exact legacy background controls still incomplete |
