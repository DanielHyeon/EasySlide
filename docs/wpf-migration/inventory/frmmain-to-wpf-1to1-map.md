...
| `WorshipListItems` | `WorshipListPanel.QueueList` (`Tag=WorshipListItems`) | partial | 3/4 | headerless `ListView/GridView`, startup default `.esw` load with DB-backed song lyrics, drag reorder, double-click Go Live, and context menu exist; `CMenuWorship_PlayOnOutput` now opens Output and starts a direct/linked media file; preview/edit/usage parity incomplete |
...
| `CMenuWorship_*` | WPF context menu | partial | 3/4 | `CMenuWorship_PlayOnOutput` is exposed and wired to Output media playback; select all, clear, edit, play, usage remain incomplete |
...
| Worship List context menu | `CMenuWorship_*` | partial | 3/4 | `Play on Output` now maps to `PlaySelectedWorshipMediaOnOutputCommand`; remaining context commands still partial |
