...
| Library WPF 화면 | 1차 구현 완료 | `LibraryWindow`, `LibraryViewModel` 추가. MainWindow에서 라이브러리 버튼으로 진입하고, AdminDB 폴더 목록, 선택 폴더의 곡 목록, 제목/대체 제목/분류/key/가사 검색, 선택 곡 가사 프리뷰, 새 곡/편집/선택 곡 이동 진입, 경로 누락/파일 누락 상태 메시지를 제공한다. 현재는 AdminDB song browse/search/edit/move 중심이며 드래그 정렬, soft delete/복구, 복사/병합 명령은 후속 구현 |
...
| Library command dialogs | 부분 구현 | `SongMoveWindow`, `SongMoveViewModel` 추가. 선택 곡과 현재 폴더를 기준으로 source 폴더를 제외한 대상 폴더를 선택하고 `IAdminDatabaseRepository.MoveSongsAsync`로 backup/transaction 기반 이동을 수행한다. AdminDB 경로/파일, 선택 곡, 대상 폴더 validation과 configured/default backup root를 자동 검증했다. 복사, 삭제/복구, 병합, 외부 복사는 후속 필요 |
