...
| Library WPF 화면 | 1차 구현 완료 | `LibraryWindow`, `LibraryViewModel` 추가. MainWindow에서 라이브러리/외부 파일/가져오기·내보내기/성경 버튼으로 진입하고, AdminDB 폴더 목록, 폴더 추가/편집 진입, 선택 폴더의 곡 목록, 제목/대체 제목/분류/key/가사 검색, 선택 곡 가사 프리뷰, 새 곡/편집/선택 곡 복사/이동/삭제 진입, 삭제 곡 복구, 스마트 병합 진입, 외부 InfoScreen/PowerPoint 파일 복사·이동 진입, Import/Export wizard 진입, 성경 버전/책/검색/선택 구절 ID 생성 진입, 폴더/곡 위아래 및 직접 drag gesture 순서 변경, 경로 누락/파일 누락 상태 메시지를 제공한다. 현재는 AdminDB folder add/edit/reorder와 song browse/search/edit/copy/move/soft delete/recover/reorder/smart merge, external file copy/move, import/export text/XML/ESF/document-folder/HTML/RTF, bible browse/search/selection 중심이며 search inspector는 후속 구현 |
...
- `dotnet test Easislides.sln -c Debug`: 335개 통과
