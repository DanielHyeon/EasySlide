...
- [x] §7.4 팔레트 항목 레벨 접근성 이름 — **완료**(2026-06-01: 명령 팔레트(⌘K) 결과의 ListBoxItem 에 합성 접근성 이름을 명시해 스크린리더가 "명령명, [위험 명령,] 분류"를 한 호흡에 읽게 한다(자식 TextBlock 합성의 UIA 비결정성 회피). `CommandDescriptor.AccessibleName` 계산 속성 + ListBox `ItemContainerStyle`(BasedOn=`{x:Type ListBoxItem}` 로 wpfui 테마 보존 + `AutomationProperties.Name`=AccessibleName). `BasedOn` 런타임 리소스 해석을 wpfui `ControlsDictionary` 로드 테스트로 **실제 검증**(MainWindow 로드 시 리소스 누락 회귀 방지). code-review 반영: 위험 배지 자식의 중복 AutomationProperties.Name 제거(중복 발화 방지, WPF엔 AccessibilityView=Raw 없음), AccessibleName 테스트 정확 일치(순서·구분자 계약). 테스트 799 green(+3). Release DLL/BAML 반영 확인)
- [x] §7.4 이름변경 고유성 검증을 폴더 관리로 확장 — **완료**(2026-06-01: 레거시 이름변경(FrmBibleRename/FrmUpdateFileName)의 고유성 규칙을 폴더 관리에 적용. 조사 결과 폴더 관리는 이미 `FolderEditorViewModel`(이름·사용여부 편집)로 존재하나 **이름 중복 검증이 없어** 같은 이름 폴더를 만들/바꿀 수 있던 갭 → `Load` 가 이미 받던 `allFolders` 로 다른 폴더 이름(자기 자신 제외, 대소문자 무시)을 모아 `Validate` 에서 중복 거부(DB 왕복 전 친절 안내). 테스트 +3(중복 거부·자기이름 허용·대소문자). **성경 버전 이름변경은 보류** — `IBibleRepository` 에 버전 write/rename 지원이 없고 WPF 버전 관리 UI 도 없어, 리포지토리 쓰기 + 관리 UI 신설이 필요한 별도 대형 기능(="이름변경 다이얼로그 확장" 범위 밖)이라 따로 다룬다.)
- [x] §7.4 FrmInfoScreen(가사 편집기) 이식 현황 정밀화 + 이식분 한글화 — **부분 완료/현황 확정**(2026-06-01: 조사 결과 FrmInfoScreen(레거시 7,337줄 = `FrmInfoScreen.cs` 3,837 + `.Designer.cs` 3,500 = 곡/가사 편집기)의 **핵심은 이미 `SongEditorViewModel`+`SongEditorWindow` 로 이식됨** — 전 곡 필드(제목/별칭/분류/키/가사/Sequence/Writer/Copyright/Capo/Timing/Notations/LicenceAdmin1·2/BookRef/UserRef/Settings/FormatData) + 미리보기(글자·배경색·폰트) + 검증(경로/폴더/제목/번호>0/Capo>=0) + 라이브 편집 안전 확인. 이번 증분: 이식된 SongEditor 의 사용자 노출 영어를 **전수 한글화** — VM 메시지 5종(Capo 검증·곡 상세 로드 실패·라이브 저장 안전확인 제목/본문/취소·프롬프트 미가용) + **미리보기 placeholder**(제목 없음·메타데이터 없음·키/카포 접두·포맷/코드/절 상태·가사 없음) + **`SongEditorWindow.xaml` 화면 라벨/툴팁/접근성 이름 전체**(제목·다른 제목·번호·분류·키·카포·가사·추가 정보·작사작곡·저작권·타이밍·교단/사용자 참조·라이선스 관리자·미리보기·저장·닫기 등). 라벨==Name 중복은 8개 입력을 LabeledBy 패턴으로 전환해 해소(접근성 스캐너 green). 테스트 +2(Capo 음수 검증·프롬프트 미가용 한글 안내). code-review(Request Changes)에서 지적한 누락(VM 1줄·XAML 라벨 17개)·문서 줄수 오류를 모두 반영. **미이식 잔여(별도 대형 과제로 명시)**: ① 절/구역 태그 시스템(`Gf.VerseTitle` 기반)과 **중복 절 표시 검증**(레거시 `FrmInfoScreen.cs:1938` "Duplicate Verse indicator") — WPF 가사엔 절 태그 파서 자체가 없음, ② 리치 per-line/구절 포맷팅 툴바, ③ 슬라이드 분할·코드/노테이션 편집 UI. 이들은 레거시 포맷 의존이 크고 범위가 방대해 단일 증분이 아닌 **독립 다증분 프로젝트로 스코핑 권장**.)
- [x] §7.4 검색 패널 필드 용어 통일(한글화) — **완료**(2026-06-01: `SongSearchQueryPanel`(SearchUsageWindow·인라인 검색 공유 컴포지트)의 영어 필드 라벨을 곡 편집기(SongEditorWindow)와 **동일 한글 용어로 통일** — 제목·가사·번호·교단 참조·사용자 참조·라이선스 관리자·작사·작곡·저작권 + 섹션(검색/키/타이밍/폴더)·플래그(코드 표기 있음/미디어 항목/수정일)·검색 버튼. 같은 필드가 화면마다 다른 언어/용어로 보이던 비일관 해소. 바인딩·AutomationProperties.Name 보존(검색 동작·기존 테스트 무회귀), 섹션 라벨(짧게)↔자동화 이름(서술적) 분리로 접근성 중복 회피. 용어 통일 잠금 테스트 +1. 테스트 805 green, code-reviewer Approve(CRITICAL/HIGH 0).)
- [x] §7.4 성경 버전 관리 UI(+리포지토리 write) 신설 — **버전 이름 변경 완료**(2026-06-01: 레거시 FrmBibleRename 대응. 성경 버전 = `Admin\Database\EsBiblesList.db` 의 `Biblefolder` 행(NAME·FILENAME·…), 본문은 별도 `HolyBibles\{FILENAME}` 파일. read-only 였던 `IBibleRepository` 에 **write 신설**: `RenameVersion`(파라미터화 UPDATE Biblefolder SET NAME WHERE FILENAME **AND DISPLAYORDER>=0** — 보이는 버전만, 본문 불변, 트랜잭션). `BibleViewModel.RenameVersion`(빈값/중복(대소문자무시, 파일명으로 자기제외)/no-op/예외 흡수 + NAME 만 바뀐 레코드 제자리 교체로 books 재조회·깜빡임 회피). 신규 `BibleVersionManagerWindow`(버전 목록 + 재사용 NameEntryWindow, ManageWorshipListsWindow 패턴) + 팔레트 런처(Window.ManageBibleVersions). 테스트 +7(repo round-trip(temp SQLite)·숨김 버전 비대상·빈값·미존재 / VM 재로드+선택보존·중복거부·미변경). 테스트 812 green, code-reviewer 2건(H1 숨김버전 가드·M2 in-place) 근본 반영. Release DLL/BAML 반영 확인. **버전 추가/삭제/순서변경은 후속**(레거시 핵심인 이름 변경에 한정).)
- [~] §7.4 FrmInfoScreen 고급 가사 포맷팅(다증분 프로젝트) — **1차: 미리보기 리치 per-line 완료**(2026-06-01: 사용자 선택 "미리보기 per-line 먼저". EasiSlides 가사의 '»'(U+00BB)는 코드/노테이션 구분 마커(레거시 gfLyrics writer 규약)임을 확인. SongEditor 미리보기를 단일 TextBlock→줄 단위 ItemsControl 로 바꿔 각 줄의 '»' 뒤 코드를 본문과 다른 스타일(작게·흐리게, 없으면 숨김)로 표시. `SongPreviewLine`(Text/Notation) + `PreviewLyricLines` + `SplitNotation`/`RebuildPreviewLines`(공통 `BuildFirstScreenLines` 추출로 기존 문자열 미리보기 무회귀). 미리보기 한정(출력 렌더·FormatData·VerseSymbol 미변경). 테스트 +3(분리/빈가사/선두마커). 815 green, code-reviewer Approve. **⚠️ 후속 정렬 필요**: 출력 렌더(LyricsDisplayFormatter)는 아직 '»'를 본문 글자로 송출 → 미리보기≠출력. 다음: ① 출력 측 '»' 처리 정렬(reader 인코딩 U+00BB vs 모지바케 "Â»" 운영 DB 확인), ② 절 태그·중복 절 검증, ③ FormatData 기반 줄별 색/정렬.)
- [x] §7.4 FrmInfoScreen 2차 — **출력 렌더 '»' 코드 처리 정렬 완료**(2026-06-01: 1차 후속 — 미리보기≠출력 불일치 해소. `LyricsDisplayFormatter.ToDisplayText`가 본문 줄의 '»'(코드/노테이션 마커) 뒤를 잘라 회중 화면엔 가사만 송출(기존 [~...] 블록 제거와 동일 원칙). 출력·미리보기 둘 다 "Â»"(U+00C2 U+00BB 모지바케)→'»' 정규화로 마커 해석 일치(미리보기=코드 흐리게 표시, 출력=코드 숨김 — 해석 동일·표시만 의도적 차이). 저장 가사 불변, ToVersePages/GetVersePage 동일 파이프라인이라 라이브 절 인덱스 정합 유지. 테스트 +4(인라인 제거·코드전용 줄 경계·절중간 코드줄 분할·모지바케). 820 green, code-reviewer Approve + stale 주석·Ordinal·회귀테스트 반영. **후속 백로그**: 본문추출 헬퍼 단일화(SplitNotation↔StripInlineNotation 중복 제거).)
- [x] §7.4 FrmInfoScreen 3차 — **Sequence 기반 절 모델 도입 완료**(2026-06-01: 사용자 선택 방향. 절을 [라벨] 마커로 1회 정의하고 Sequence("1 C 2 C")로 순서·반복 지정(레거시 절 순서 대응). `LyricsDisplayFormatter`에 `ToVersePages(lyrics, sequence)`/`GetVersePage(lyrics, idx, sequence)` 오버로드 + `ParseLabeledSections`(라벨→정리 본문) + `TryExpandBySequence`(토큰 매칭 확장, 매칭 0이면 null→선형 폴백). `LiveQueueItem.Sequence` + `AddSong(song, sequence)` + `AddSongByIdAsync`가 `detail.Sequence` 전달 + 라이브 호출부 3곳(절 페이지 수·본문) 배선 → 종단 동작. **무회귀**: 단일인자 오버로드·기본 null로 기존 선형 경로 비트 동일, 레거시 char-인코딩 Sequence는 매칭 0→선형 안전 폴백. 테스트 +8(확장·구분자·빈/무매칭 폴백·GetVersePage·AddSong 적재·선형·종단 통합). 828 green, code-reviewer 2라운드(CRITICAL: detail.Sequence 미전달 데드코드 → 전달+종단 통합 테스트로 수정 → Approve). **이제 중복 절 검증이 의미 성립**(절 1회 정의 모델). **후속**: 중복 절 라벨 검증(SongEditor 경고) / 정규화 헬퍼 단일화 / 부분매칭 미매칭 토큰 가시화)
- [x] §7.4 FrmInfoScreen 4차 — **중복 절 라벨 검증 완료**(2026-06-01: Sequence 모델(절 1회 정의) 위에서 성립. 같은 [라벨]이 두 번 정의되면 둘째가 시퀀스 첫-매칭에서 무시되는 작성 오류 → 경고. `LyricsDisplayFormatter.FindDuplicateSectionLabels`(대소문자 무시, 처음 본 표기로 중복당 1회, [~..] 노테이션 제외, 기존 SectionLabel/IsMarkerOnlyLine 재사용) + `SongEditorViewModel.SectionWarning`(RefreshPreview 실시간 갱신) + SongEditorWindow 하단 경고색 표시. **비차단**(저장 안 막음 — 편집 과도기 고려). 레거시 FrmInfoScreen.cs:1938 "Duplicate Verse indicator" 대응. 테스트 +6. 834 green, code-reviewer Approve + MEDIUM(FindDuplicate Â» 정규화 누락→추가·3회테스트) 반영.)
- [x] §7.4 FrmInfoScreen 5차 — **레거시 v32 FormatData 디코더 + 미리보기 색 적용 완료**(2026-06-01: 사용자 선택 "레거시 v32 디코더 구축". 곡별 `FORMATDATA`("code=value>code=value>…" 사전, 레거시 `gfLyrics.cs` HeaderData[n] 권위)를 파싱하는 `SongFormatData`(record + `Parse`/`ArgbToHex`). 필드 매핑: 29/30=글자색·26/27=배경색 region1/2(ARGB signed int→`#AARRGGBB`), 43/44=폰트명·47/48=크기(**6~100 유효**), 31/32=정렬(1~3), **41=글꼴효과 비트**(region1 bit1=Bold·bit2=Italic, region2 bit4·5), 61=배경이미지·51=미디어. 숫자 키만 인식·미지 키 무시·빈/공백→null. SongEditor 미리보기가 FormatData region1 글자/배경색을 우선 적용(없으면 설정 폴백). **실 운영 DB(`C:\EasiSlides`)에서 SONGID 454 실 샘플 확보해 검증**. 테스트 +8(실샘플 전필드·ArgbToHex·빈/공백·잘린키·field41 비트·실샘플 41=16→region2 Italic·미리보기 색). 842 green, code-reviewer 2라운드(**CRITICAL: Bold/Italic 소스 필드 45→41 교정**(45는 ShowFontVPosition), MAJOR: 폰트 크기 가드 >0→6~100, MINOR: 테스트/주석 field45→41 + 실샘플 비트 단언 — 모두 레거시 권위·실데이터로 근본 수정) → Approve. Release DLL 심볼 반영 확인. **후속**: 라이브 출력 측 per-region FormatData 적용(색/정렬/폰트/배경, region2는 [region 2] 마커) / 정렬·폰트·배경이미지 미리보기 반영.)
- [x] §7.4 FrmInfoScreen 6차 — **라이브 출력 per-song FormatData 색 적용 완료**(2026-06-01: 5차 디코더(`SongFormatData`)를 곡 편집기 미리보기에만 쓰던 것을 **실제 회중 송출 출력까지 관통**. 곡이 라이브로 나가면 그 곡의 region1 글자색·배경색으로 표시(없으면 운영 기본색 — 무회귀). 데이터 흐름: `SongDetail.FormatData`→`MainViewModel.AddSong(…, formatData)`→`LiveQueueItem.FormatData`→`LiveSessionService.GoLive`가 `SongFormatData.Parse`로 region1 색 디코드→`LiveSessionSnapshot.OverrideTextColorArgb/OverrideBackgroundColorArgb`(둘 다 nullable, 기본 null)→`OutputRenderer.CreateScene`가 **kind==Live 이고 오버라이드가 있으면** 운영 기본색 대신 곡 색을 scene 색 필드에 채움(배경은 솔리드: 끝색=시작색·그라데이션 해제). ApplyScene/XAML 무변경(scene 색 필드를 그대로 소비). **무회귀**: 새 필드 전부 기본 null→기존 곡(FormatData 없음)·비-Live·PPT/미디어 경로 비트 동일, record 동등성은 색 전환 시에만 이벤트 발생(정상). 비-Live(숨김/블랙아웃)는 게이트로 무시+검정 오버레이로 차단, Restore 시 색 정확 복원. 테스트 +6(GoLive 적재/null·렌더러 Live 우선/비-Live 무시·AddSong 직접/검색 종단). 848 green, code-reviewer **Approve**(CRITICAL/MAJOR 0; 레이어링·게이트·동등성·무회귀 모두 적정 확인). Release DLL 심볼 반영 확인. **후속(백로그)**: ① 정렬(31/32)·폰트(43/47)·배경이미지(61) 출력 적용, ② region2 [region 2] 마커 이중언어 적용, ③ 배경만 있고 글자색 없는 곡 가독성 폴백(MINOR), ④ 다중 오버라이드 확장 시 SongOverride nested record 로 묶기(MINOR).)
- [x] §7.4 FrmInfoScreen 7차 — **라이브 출력 per-song 가로 정렬 적용 완료**(2026-06-01: 6차 색 오버라이드와 **동일 메커니즘**으로 곡별 FormatData 가로 정렬(코드 31, region1)을 라이브 출력까지 관통. 곡이 라이브로 나가면 그 곡의 정렬로 가사 송출(없으면 운영 기본 정렬 — 무회귀). `LiveSessionSnapshot.OverrideTextAlignment`(`LyricsTextAlignment?`, 기본 null) + `GoLive` 가 `MapAlignment(format?.Alignment1)`(레거시 **1→Left(Near)·2→Center·3→Right(Far)**, 범위밖/null→null) 로 매핑 적재 → `OutputRenderer.CreateScene` 가 색과 **동일 `isLive` 게이트**로 곡 정렬 우선. 레거시 권위 `gfLyrics.cs:191-202`(case 3→Far·1→Near·default→Center)와 매핑 정확 일치 확인. **무회귀**: 미지정 곡·비-Live 비트 동일. 테스트 +7(정렬 매핑 1/2/3·필드부재·범위밖(31=9) 종단 null·렌더러 Live 우선/비-Live 무시). 855 green, code-reviewer **Approve**(CRITICAL/MAJOR 0; 레거시 매핑 일치·게이트 일관·무회귀 확인) + MINOR 2(region1 한정 주석·범위밖 종단 테스트) 반영. Release DLL 심볼 반영 확인. **후속(백로그)**: 폰트명(43)·크기(47, pt↔px 단위 처리)·배경이미지(61) 출력 적용 / region2 이중언어 정렬.)
- [x] §7.4 정리(리팩토링) — **가사 정규화 NormalizeText 헬퍼 단일화 완료**(2026-06-01: 동작-보존 리팩토링. 줄바꿈(\r\n·\r→\n)+코드 마커 모지바케("Â»" U+00C2 U+00BB→'»' U+00BB) 정규화 블록이 4곳에 중복(`LyricsDisplayFormatter` 의 ToDisplayText·ToVersePages 중복검사·ParseLabeledSections + `SongEditorViewModel.BuildFirstScreenLines` 미리보기)되어 있던 것을 `LyricsDisplayFormatter.NormalizeText`(internal static) 단일 규칙으로 통합. 이전엔 미리보기≡출력 정규화 일치를 **주석으로만** 보장(한쪽만 바뀌면 조용히 어긋날 위험) → 이제 코드로 강제. **TDD 안전망**: 리팩토링 전 기존 미검증 진입점 2곳에 정규화 특성화 테스트 추가(FindDuplicate CRLF / 시퀀스 경로 CRLF+모지바케)하고 *리팩토링 전 통과*를 먼저 확인(동작 동일 입증). 857 green, code-reviewer **Approve**(CRITICAL/MAJOR 0; 순서·Ordinal·nullable flow 보존 확인) + MINOR 2(주석 위치 정리·4번째 사본 통합)를 즉시 반영(4번째 사본도 이번에 통합). Release DLL 심볼 반영 확인.)
- [x] §7.4 정리(한글화) — **SearchUsageWindow 자체 크롬·DataGrid 한글화 완료**(2026-06-01: 검색/사용 창의 영어 잔존 표면을 곡 편집기·검색 패널과 **동일 한글 용어로 통일**. 창 제목(검색 및 사용 현황)·탭(곡/제목/사용 현황)·버튼(새로고침/조회/불러오기/삭제/RTF 내보내기/닫기)·DataGrid 헤더 12종(제목·폴더·번호·키·일치 항목·발췌·다른 제목·교단 참조·사용자 참조·날짜·예배 순서·횟수). **순수 표시 문자열만 변경** — 바인딩·Command·AutomationProperties.Name(기존) 무변경이라 검색/사용 동작 무회귀(기존 SearchUsageViewModelTests 가 보장). 신규 한글화 잠금 테스트 3개(XAML 구조 파싱, 한글 존재+영어 부재 양방향 단언 — 회귀 시 영어 재유입 차단). 860 green, code-reviewer **Approve**(CRITICAL/MAJOR 0; 용어 100% 일치·기능 무변경·양방향 잠금 확인) + MINOR(RTF 버튼 접근성 이름·툴팁) 반영. Release BAML 한글 임베드 확인. **잔여 미세 불일치**(의도 유지): MainWindow 인라인 레일 탭은 공간 제약상 "사용"(이 창은 "사용 현황").)
- [x] §7.4 FrmInfoScreen 8차 — **라이브 출력 per-song 폰트(43/47)·배경이미지(61) 적용 완료**(2026-06-01: 6·7차 색·정렬 오버라이드와 **동일 메커니즘**으로 곡별 FormatData 글꼴·배경이미지를 라이브 출력까지 관통. ① **폰트명(43)**: `LiveSessionSnapshot.OverrideFontName`(string?, 공백→null=테마 상속) → `OutputSceneSnapshot.LyricsMonitorFontFamily` → `OutputWindowViewModel.BodyFontFamily`(**object = FontFamily 또는 `DependencyProperty.UnsetValue`** — 오버라이드 없으면 UnsetValue 바인딩으로 부모(테마) 글꼴을 그대로 상속, null 로 떨어뜨려 기본 글꼴로 바뀌는 회귀 방지) → 본문 TextBlock+OutlinedTextBlock `FontFamily` 바인딩. ② **폰트 크기(47)**: 레거시 pt(WinForms 기본) → WPF px(DIP) **×96/72** 변환(`LegacyPointToPixel`, 8~240 방어 클램프) → `OverrideFontSizePx` → 씬 `LyricsMonitorFontSize` 재사용(줄 높이도 자동 비례). ③ **배경이미지(61)**: `OverrideBackgroundImagePath` → 씬 `BackgroundImagePath` → VM 이 GAP 로고 로더 재사용(경로 해석: 절대=그대로, 상대=작업폴더 기준 + 캐시)로 로드 → `OutputWindow.xaml` 최하단 `Image`(RowSpan=3, 색 배경 위·콘텐츠 아래, 이미지 우선) + `BackgroundImageVisibility`. 로드 실패(파일 없음)면 색 배경으로 안전 폴백. 모두 **`kind==Live` 게이트**라 비-Live·미오버라이드·PPT/미디어 경로는 비트 동일(무회귀). FormatData 파이프라인(SongDetail→AddSong→LiveQueueItem→GoLive)이 이미 깔려 있어 종단 자동 배선. 테스트 +15(폰트명/크기 적재·pt→px·공백폰트·렌더러 Live 우선/비-Live 무시·VM FontFamily 매핑·배경 로드/폴백). code-reviewer **Approve**(CRITICAL/MAJOR 0; 무회귀 게이트·UnsetValue 상속 idiom·z-order 검증). **후속(백로그)**: region2 [region 2] 이중언어 폰트 / 배경이미지 Best Fit·Centre·Tile 모드(현재 UniformToFill 고정) / 제목 헤딩 곡 글꼴 적용 여부.)
- [x] §7.4 성경 버전 관리 — **추가·삭제·순서변경 완료**(2026-06-01: 이름 변경(레거시 FrmBibleRename)에 이어 read-only 였던 `IBibleRepository` 에 **write 3종 신설**(파라미터화 SQL+트랜잭션). ① **순서변경**: `ReorderVersions`(주어진 파일 순서대로 DISPLAYORDER 0,1,2… 재기록, 숨김/미지 파일은 번호 미소모·건드리지 않음) + VM `MoveVersionUp/Down`(경계 false·재로드 후 선택 보존). ② **삭제**: `DeleteVersion`(DISPLAYORDER=-1 **숨김** — 본문 파일·행 보존, 비파괴적·되돌릴 수 있음, 레거시 "삭제 예정" 의미) — 운영 실수 방지 확인 다이얼로그. ③ **추가**: `GetAddableVersions`(숨김 행 + HolyBibles\*.db 신규 파일) + `AddVersion`(숨김 복구=UPDATE / 신규=INSERT, **이미 보이는 파일은 거부**(code-review MINOR 반영)·파일 없으면 거부) + VM 중복 이름 검증(대소문자 무시) + 신규 `BibleVersionAddWindow` 후보 picker(제안 이름 자동 채움·인라인 중복 안내). 기존 `BibleVersionManagerWindow` 에 ↑/↓/추가/삭제 버튼 추가(SelectedItem 양방향 바인딩으로 선택 추종). 팔레트 런처(Window.ManageBibleVersions) 그대로 재사용. 테스트 +17(repo 순서/삭제숨김/추가후보/추가·복구/이미보임거부/빈값·미존재 round-trip(temp SQLite) + VM 이동·삭제·추가·중복·후보). code-reviewer **Approve**(CRITICAL/MAJOR 0; SQL 주입 0·트랜잭션 원자성·DISPLAYORDER 엣지 검증). **후속**: 신규 파일 import(파일 복사) UI / DISPLAYORDER 갭 정리.)
- [ ] **다음 착수(권장)**: region2 [region 2] 이중언어(색·정렬·폰트) 출력 적용 / 배경이미지 표시 모드(Best Fit·Centre·Tile) / 성경 신규 파일 import UI
