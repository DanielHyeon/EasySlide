...
- [ ] **다음 착수(권장)**: FrmInfoScreen 고급 가사 포맷팅(절 태그·중복 절 검증·per-line 포맷) 다증분 프로젝트 / 성경 버전 관리 UI(+리포지토리 write) 신설 후 이름변경 연결 / SearchUsageWindow 자체 크롬·탭 헤더·DataGrid 컬럼 한글화(별도 증분)
