...
| 항목별 인라인 포맷(`Ind_*` 즉시 반영) | **증분106 절 순서 인라인 편집** + 우클릭 개별서식 토글 | 🟡 선택 곡의 **절 순서(Sequence) 인라인 편집**(예배 순서 패널 입력칸 — "1 2 C 3 C C", 즉시 재페이지네이션·라이브 재송출, 곡만 활성) + 항목 우클릭 "개별 서식 사용" 토글. 항목별 색·글꼴·정렬 인라인 즉시 편집은 후속 |
| 더블클릭 미리보기→편집 | — | 🔴 |
