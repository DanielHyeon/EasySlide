...
| FrmMain 기능 | WPF | 상태 |
|---|---|---|
| **Region 1/2 이중 언어**(영역별 색·정렬·폰트·크기·세로위치·굵게·기울임·밑줄) | `[region 2]` 마커 파서(GetRegionPages) + 영역별 송출(Region1 위 Region2) + 영역별 색/정렬/글꼴/크기/굵게/기울임/밑줄·간격·인터레이스·표시모드 | 🟢 이중 언어 출력 렌더 동작(이번 다증분 슬라이스의 최대 갭 해소). 영역별 세로위치만 본문 전체 오프셋(증분73)으로 근사 |
...
| InfoScreens(.esi) 목록·편집 | 명명 정보화면 목록(InfoScreenStore — 이름별 저장/불러오기/삭제) + 본문·글자크기·**가로 정렬**(증분77) | 🟡 (증분76·77) 명명 저장·텍스트/크기/정렬 편집. 풀 편집기(7337줄)·.esi 레거시 포맷은 후속 |
