...
| `Ind_R1Bold/Italics(3종)/Underline`, R2 동일 | 영역별 굵게/기울임(없음/전체/후렴만)/밑줄 | 🟢 (증분 46/47) 영역별 굵게/기울임 + (증분 48) 밑줄 전역 토글 + (증분 49) **영역별 밑줄** + (증분 50) **외곽선 모드 밑줄**(OutlinedTextBlock 형상에 밑줄 포함). ChorusOnly(후렴만)만 후속. 레거시 비트 한계로 굵게/기울임/밑줄 "끄기"는 전역 추종 |
