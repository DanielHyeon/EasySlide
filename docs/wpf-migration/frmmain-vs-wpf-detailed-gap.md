...
| Images(`flowLayoutImages`, Scenery/Tiles, 배경 적용) | 이미지 갤러리(ImageLibraryWindow — 폴더 썸네일·출력 배경 적용/지우기·카테고리·**파일명 검색**) | 🟢 폴더 이미지를 썸네일로 보고 출력 배경으로 적용 + 카테고리(Scenery/Tiles 하위폴더) 필터 + **파일명 검색**(증분133, 카테고리와 AND). 검색 결과 개수 상태줄 표시만 후속(비동기 로딩과 상태줄 소유 경쟁 회피) |
...
| InfoScreens(.esi) 목록·편집 | 명명 정보화면 목록(InfoScreenStore 저장/불러오기/삭제) + 본문·글자크기·가로 정렬·글자색(76~78)·**배경색(증분124)**·**강조 굵게/기울임/밑줄(증분125)**·**글꼴명(증분126)** | 🟡 명명 저장·텍스트/크기/정렬/글자색/**배경색**/**강조**/**글꼴** 편집(곡 FormatData 코드26·코드41 강조·**코드43 글꼴명** 파이프라인 재사용 — 송출 시 OverrideBackgroundColorArgb·OverrideBold1/Italic1/Underline1·**OverrideFontName** 으로 종단 렌더. 글꼴은 가사 글꼴 콤보와 같은 목록 공유, 편집 콤보라 직접 입력도 됨). 풀 편집기(7337줄)·.esi 레거시 포맷만 후속 |
...
| Gap Item 처리 | GapItem 렌더(None/Black/Default/User 모드 + 페이드 + User 로고) + **출력 메뉴 "대기 화면(Gap)" 조작 UI(증분120)** | 🟢 대기 화면 Gap 모드 4종 렌더(증분119 Black=진짜 검은 화면) + **출력 메뉴에서 모드 콤보·페이드 토글·로고 이미지 픽커/지우기 즉시 조작**(로고 고르면 모드 자동 User 전환, 설정 라이브 갱신). 설정 창과 양방향 동기화 |
...
| 항목별 인라인 포맷(`Ind_*` 즉시 반영) | **증분106~113 절 순서 + 글자색/정렬/크기/글꼴/배경색/배경이미지/강조** + 우클릭 개별서식 토글 | 🟢 선택 곡의 **절 순서**(입력칸 "1 2 C 3 C C") + 우클릭 **글자색**(29)·**정렬**(31)·**글자 크기**(47)·**글꼴명**(43)·**배경색**(26)·**배경 이미지**(코드61)·**강조 굵게/기울임/밑줄**(코드41 비트 토글) — **8종 인라인 서식**을 공통 헬퍼(Parse→한 필드만 바꿔 Encode, 나머지 보존)로 FormatData 편집, 주면 개별서식 자동 켬, 즉시 라이브 재송출, **곡·성경 활성**(증분128 — 성경도 본문 구절을 렌더하고 GoLive 가 Kind 무관이라 곡과 동일 적용. 절 순서 편집은 곡만. 강조는 성경에 후렴 라벨이 없어 "강조 후렴만" 켜질 땐 억제—색·정렬·글꼴은 무관) + "개별 서식 사용" 토글. **이중 언어(보조 영역 Region2) 5종 완료**: 우클릭 "이중 언어(보조 영역)" 하위 메뉴에 **글자색**(30)·**정렬**(32)·**글자 크기**(48)·**글꼴**(44)·**강조**(굵게/기울임/밑줄, 코드41 상위비트) — 비우면 본문 추종, 이중언어 곡만 보임, 후렴만 게이트 적용. (메뉴 캐스케이드 깊어 전용 다이얼로그 승격은 백로그.) 항목 미디어 배정(코드51)은 출력 미소비라 미노출(정직) |
| 더블클릭 미리보기→편집 | — | 🔴 |
