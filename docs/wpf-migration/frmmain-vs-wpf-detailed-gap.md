...
| 항목별 인라인 포맷(`Ind_*` 즉시 반영) | **증분106 절 순서 + 107~111 글자색/정렬/크기/글꼴/배경색** + 우클릭 개별서식 토글 | 🟢 선택 곡의 **절 순서**(입력칸 "1 2 C 3 C C") + 우클릭 **글자색**(코드29)·**정렬**(코드31)·**글자 크기**(코드47)·**글꼴명**(코드43)·**배경색**(어두운 프리셋5+해제→코드26) — 6종 인라인 서식을 공통 헬퍼(Parse→한 필드만 바꿔 Encode, 나머지 보존)로 FormatData 편집, 주면 개별서식 자동 켬, 즉시 라이브 재송출, 곡만 활성 + "개별 서식 사용" 토글. 항목별 배경 이미지(코드61)만 후속 |
| 더블클릭 미리보기→편집 | — | 🔴 |
