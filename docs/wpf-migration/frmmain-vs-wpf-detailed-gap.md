...
| 항목별 인라인 포맷(`Ind_*` 즉시 반영) | **증분106 절 순서 + 증분107 항목별 글자색** + 우클릭 개별서식 토글 | 🟡 선택 곡의 **절 순서(Sequence) 인라인 편집**(입력칸 "1 2 C 3 C C") + **항목별 글자색**(우클릭 "이 항목 글자색" 프리셋 5색+해제 → FormatData 코드29, 색 주면 개별서식 자동 켬, 즉시 라이브 재송출, 곡만 활성) + "개별 서식 사용" 토글. 항목별 글꼴·정렬·배경 인라인 즉시 편집은 후속 |
| 더블클릭 미리보기→편집 | — | 🔴 |
