...
7. **placeholder 더미 큐** — 시작 시 가짜 항목 3개 시드(`SeedPlaceholderQueue`). 실제 항목/큐 도메인 plumbing 미완.
...
9. **이미지/배경 라이브러리·배경 표시 모드 부재** — 배경 운영이 곡별 FormatData 이미지(UniformToFill 고정)뿐.
