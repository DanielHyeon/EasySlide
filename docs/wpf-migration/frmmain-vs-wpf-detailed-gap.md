...
| 항목별 인라인 포맷(`Ind_*` 즉시 반영) | **증분106 절 순서 + 107~110 글자색/정렬/크기/글꼴** + 우클릭 개별서식 토글 | 🟡 선택 곡의 **절 순서**(입력칸 "1 2 C 3 C C") + 우클릭 **글자색**(프리셋5색→코드29)·**정렬**(왼/가운데/오른+해제→코드31)·**글자 크기**(작게~아주크게 pt+해제→코드47)·**글꼴명**(자주쓰는 한/영 5종+해제→코드43) — 색/정렬/크기/글꼴은 공통 헬퍼(Parse→한 필드만 바꿔 Encode, 나머지 서식 보존)로 FormatData 편집, 주면 개별서식 자동 켬, 즉시 라이브 재송출, 곡만 활성 + "개별 서식 사용" 토글. 항목별 배경(색·이미지) 인라인은 후속 |
| 더블클릭 미리보기→편집 | — | 🔴 |
