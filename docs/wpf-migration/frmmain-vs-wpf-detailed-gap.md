...
| FrmMain 기능 | WPF | 상태 |
|---|---|---|
| **Region 1/2 이중 언어**(영역별 색·정렬·폰트·크기·세로위치·굵게·기울임·밑줄) | `[region 2]` 마커 파서(GetRegionPages) + 영역별 송출(Region1 위 Region2) + 영역별 색/정렬/글꼴/크기/굵게/기울임/밑줄·간격·인터레이스·표시모드 + **Region2 전역 글꼴(증분80)·전역 색(증분82)** | 🟢 이중 언어 출력 렌더 동작(이번 다증분 슬라이스의 최대 갭 해소). Region2 전역 글꼴·색 설정(0/빈값=본문 추종, 곡별 44/30 우선). 영역별 세로위치만 본문 전체 오프셋(증분73)으로 근사 |
