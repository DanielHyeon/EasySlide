...
| Gap Item 처리 | GapItem 렌더(None/Black/Default/User 모드 + 페이드 + User 로고) | 🟡 대기 화면 Gap 모드 4종 렌더(**증분119: Black=진짜 검은 빈 화면 수정** — 기존엔 운영 기본 배경에 "BLACK" 글자만). 모드/로고는 설정 창에서 설정. Gap 모드 빠른 토글/로고 픽커를 메인 UI 로 노출(조작 UI)은 후속 |
