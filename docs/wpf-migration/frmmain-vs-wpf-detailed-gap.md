...
| InfoScreens(.esi) 목록·편집 | 명명 정보화면 목록(InfoScreenStore 저장/불러오기/삭제) + 본문·글자크기·가로 정렬·글자색(76~78)·**배경색(증분124)**·**강조 굵게/기울임/밑줄(증분125)** | 🟡 명명 저장·텍스트/크기/정렬/글자색/**배경색**/**강조** 편집(곡 FormatData 코드26·**코드41 강조 비트밭** 파이프라인 재사용 — 송출 시 OverrideBackgroundColorArgb·**OverrideBold1/Italic1/Underline1** 로 종단 렌더, "강조 후렴만" 게이트 기본 꺼짐이라 공지도 굵게 보임). 풀 편집기(7337줄)·.esi 레거시 포맷만 후속 |
