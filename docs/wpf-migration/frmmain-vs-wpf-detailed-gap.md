...
| 창 상태(splitter/bounds) 레지스트리 저장 | **메인 창 크기·위치·최대화 저장/복원(증분88)** + **인스펙터 접힘 상태 저장(증분89)** — 화면 밖/모니터 분리/해상도 변경 보정 순수 헬퍼 | 🟡 메인 창 bounds + 인스펙터 펼침/접힘 저장·복원(닫을 때 저장, 열 때 화면 안으로 보정). 좌측 패널 splitter 높이 저장만 후속 |
