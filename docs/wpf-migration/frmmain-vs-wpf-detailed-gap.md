...
| 항목별 인라인 포맷(`Ind_*` 즉시 반영) | **증분106 절 순서 + 107 글자색 + 108 정렬** + 우클릭 개별서식 토글 | 🟡 선택 곡의 **절 순서**(입력칸 "1 2 C 3 C C") + **항목별 글자색**(우클릭 프리셋5색+해제 → 코드29) + **항목별 정렬**(우클릭 왼쪽/가운데/오른쪽+해제 → 코드31) — 색·정렬은 공통 헬퍼(Parse→한 필드만 바꿔 Encode, 나머지 서식 보존)로 FormatData 편집, 주면 개별서식 자동 켬, 즉시 라이브 재송출, 곡만 활성 + "개별 서식 사용" 토글. 항목별 글꼴·크기·배경 인라인은 후속 |
| 더블클릭 미리보기→편집 | — | 🔴 |
