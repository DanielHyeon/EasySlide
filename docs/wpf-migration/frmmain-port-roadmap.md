...
- (2026-06-01) ✅ 증분 24(하트) — **하트(Heart)**. 2-레이어 엔진 위에 매개변수 곡선 폴리라인(90점)으로 정확한 하트 도형 추가(베지어 추정 없음). 위 노치·코너는 옛 프레임, 완료 시 새 화면. 메뉴 **모션 25종**. FillContains(본체·코너·노치·닫힘) 단위 테스트. 1157 green. 커밋 `f909666`. **남은 도형(P2, 장식)**: Spiral/WindMill/FanUp 등 회전·각도 기반 도형(동일 엔진 위 각도 애니메이션 추가).
