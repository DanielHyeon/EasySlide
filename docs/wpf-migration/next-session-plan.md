...
### D. 접근성 2차 정밀화 (§7.3 보강) — **저위험**
1차 스윕은 `AutomationProperties.Name`을 부여했다. 추가로:
- 입력 필드를 인접 라벨에 `AutomationProperties.LabeledBy`로 연결(현재는 Name 직접 부여).
- 라이브 상태 동적 영역 `LiveRegion="Assertive"` 적용(§7.3) — LiveBar/EsToast.
- `Controls/` 템플릿 파트의 자동화 트리 노출 정책 정리(현재 스캐너 제외).

## 2. 권장 착수 순서

1. **D(접근성 2차)** — 저위험, 1차 스윕 연장선. 워밍업.
2. **C(스크린샷 회귀)** — 이후 리팩토링 안전망이 되므로 먼저 구축.
