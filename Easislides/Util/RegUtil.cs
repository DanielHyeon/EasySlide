...
namespace Easislides.Util
{
    class RegUtil
    {
		private const string RegistryBasePath = "Software\\EasiSlides\\";
		...
		{
			SaveRegValueInternal(Section, Name, Value, RegistryValueKind.String);
		}

		public static void SaveRegValue(string Section, string Name, int Value)
		{
			SaveRegValueInternal(Section, Name, Value, RegistryValueKind.DWord);
		}

		public static void SaveRegValue(string Section, string Name, long Value)
		{
			SaveRegValueInternal(Section, Name, Value, RegistryValueKind.QWord);
		}

		private static void SaveRegValueInternal(string Section, string Name, object Value, RegistryValueKind valueKind)
		{
			try
			{
				using (RegistryKey registryKey = Registry.CurrentUser.CreateSubKey(RegistryBasePath + Section))
				{
		...
		public static void DeleteRegKey(string Section, string Name)
		{
			try
			{
				using (RegistryKey registryKey = Registry.CurrentUser.OpenSubKey(RegistryBasePath + Section, writable: true))
				{
		...
		// Obsolete: Use DeleteRegKey instead
		[System.Obsolete("Method name typo. Use DeleteRegKey instead.")]
		public static void DeleletRegKey(string Section, string Name)
		{
			DeleteRegKey(Section, Name);
		}
	}
}
