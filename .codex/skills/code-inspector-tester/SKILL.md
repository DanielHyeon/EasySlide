...
For this codebase, adapt inspection and test strategy to the actual stack in use:
- **React 19.2 / TypeScript frontend**
  - Verify component tests use React Testing Library patterns and focus on user behavior
  - Watch for stale React 18-era assumptions, especially effect-heavy logic and unnecessary `useMemo` / `useCallback`
  - If React Compiler is enabled, do not recommend manual memoization unless profiling justifies it
- **Next.js 16 frontend**
  - Distinguish Server Components, Client Components, route handlers, and Server Actions in test strategy
  - Verify loading/error boundaries, form submission states, and cache/rendering behavior
- **Spring Boot 3.x backend**
  - If using WebFlux, verify Reactive streams are properly tested with StepVerifier and that blocking calls do not leak into reactive chains
  - If using MVC, prefer controller/service/repository test slices and integration tests for security and persistence behavior
  - Validate security annotations such as `@PreAuthorize` have corresponding tests
- **FastAPI backend**
  - Verify request/response validation, async behavior, dependency injection, and auth failures are covered with pytest/httpx-style tests
  - Check adherence to the Python baseline: FastAPI + Pydantic v2 + Granian + Polars + Ruff + uv + orjson
  - Flag unnecessary pandas usage in performance-sensitive data paths if Polars is the intended default
- **Node.js backend**
  - Verify async error handling, startup/config validation, route contracts, and shutdown behavior are tested
- **Data layer**
  - Ensure repository/ORM/database access patterns match the selected stack instead of forcing reactive assumptions everywhere
