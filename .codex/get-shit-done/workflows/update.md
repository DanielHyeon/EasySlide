...
for cand in \
  ...
  "E:/WorkSpace/wp-ms/dev/EasiSlides_v2.6.4/.codex/get-shit-done/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`E:/WorkSpace/wp-ms/dev/EasiSlides_v2.6.4/.codex/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just the agent. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `E:/WorkSpace/wp-ms/dev/EasiSlides_v2.6.4/.codex/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
