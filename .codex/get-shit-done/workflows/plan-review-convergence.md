...
```bash
INIT=$(node "E:/WorkSpace/wp-ms/dev/EasiSlides_v2.6.4/.codex/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "E:/WorkSpace/wp-ms/dev/EasiSlides_v2.6.4/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "E:/WorkSpace/wp-ms/dev/EasiSlides_v2.6.4/.codex/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
