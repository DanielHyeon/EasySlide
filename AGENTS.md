...
5. 승인된 Phase **하나만** TDD(Red→Green→Refactor→Verify), 빌드 + `dotnet test` green 유지.
6. `/gstack-review` → `/gstack-cso` → `/gstack-qa` → DoD 확인 → 수동 송출 QA.
7. `/opsx:sync` → `/opsx:archive` → **`gbrain_store`로 교훈·구조·의사결정 영구 저장** → `/gstack-ship`(게이트만, 실제 push는 사람).

**B. 불변 가드**

- **Single Writer**: 여러 에이전트로 조사·리뷰를 병렬화하더라도(네이티브 Workflow/Agent 포함) 같은 파일/브랜치 **쓰기는 한 에이전트만**.
- OpenSpec 승인·CodeGraph impact·검증 증거 요구는 그대로 유지. 송출 좌표/DB/Interop 같은 High-risk는 **조사만 병렬, 수정·검증은 단일 작업자 + full 게이트**.
