...
namespace OfficeLib
{
    ...
    public class PowerPoint
	{
		...
		//레지스트리에 등록 되는 Screen Device Name
		...
		/// <summary>
		/// daniel
		/// 이 프로그램에서 사용한 파워포인트만 닫음
		/// 다른 작업 때문에 연 파워포인트 파일은 닫지 않음
		/// </summary>
		...
		/// daniel 
		/// 파워포인트 Slide Show가 될때 윈도우 창을 Minimized 상태로 바꾼다.
		/// </summary>
		public void NewApp()
		{
			try
		...
		public bool BuildScreenPreDumps(string FilePath, string FilePrefix, ref int TotalSlides, int MAX_VERSES, int DB_MAXSLIDES, ref int[] SongVerses, ref int[,] Slide, string[] SequenceSymbol)
		{
			...
			try
			{
				...
				try
				{
					/// 수정Slide 카운트, SongVerses 카운트 만큼만 초기화
        ...
        public void ClosePresentation(ref _Presentation presentation)
        {
            try
            {
                ...
                {
                    presentation.Close();
                    presentation.Dispose();  // NetOffice에서 제공하는 Dispose() 사용
                }
            }
            catch (Exception e)
            {
        ...
        public void QuitPowerPointApp(Application prePowerPointApp)
        {
            try
            {
                if (prePowerPointApp != null && !prePowerPointApp.IsDisposed)
                {
                    ...
                    prePowerPointApp.Dispose();  // Dispose()는 Quit() 이후에 호출
                }
            }
