...
public sealed record LiveSessionSnapshot(
    ...
    // 레거시 BOOK_REFERENCE/USER_REFERENCE — Reference alert source 3/4 에 사용.
    string CurrentItemBookReference = "",
    string CurrentItemUserReference = "",
    ...
    LyricsBackgroundMode? OverrideBackgroundImageMode = null,
...
public sealed class LiveSessionService : ILiveSessionService
{
        ...
        bool isCleared)
    {
        ...
        return new LiveSessionSnapshot(
            ...
            CurrentItemBookReference: item.BookReference,
            CurrentItemUserReference: item.UserReference,
            ...
            OverrideBackgroundImageMode: format?.BackgroundImageMode,
    ...
    private static string ComputeCurrentSectionLabel(LiveQueueItem item)
    {
        ...
        if (LyricsDisplayFormatter.HasRegion2(item.Lyrics))
        {
            ...
            // 페이지 수와 1:1 정렬될 때만 신뢰한다(어긋나면 빈 문자열 — 엉뚱한 절 라벨 표시 방지).
        ...
        {
            // 단일 영역 라벨은 ToVersePages 와 1:1 정렬이 보장된다(라벨 없는 절은 빈 문자열).
    ...
    // 현재 송출 절이 후렴인지 — "강조 후렴만"(ChorusOnly) 효과 판정용. 위 섹션 라벨 계산을 재사용한다.
    private static bool ComputeCurrentPageIsChorus(LiveQueueItem item)
        => LyricsDisplayFormatter.IsChorusLabel(ComputeCurrentSectionLabel(item));
